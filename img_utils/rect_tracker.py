...
def groups(glist, numPerGroup=2):
    ...
    # i = 0
    cur = []
    for i, item in enumerate(glist):
        if i % numPerGroup == 0 and i > 0:
            result.append(cur)
            cur = []
        cur.append(item)
...
class RectTracker:
    ...
    def cool_design(self, event):
        ...
        dashes = [3, 2]
        # todo replace 1000 with canvas scrollregion
        self.x = self.canvas.create_line(event.x, 0, event.x, 1000, dash=dashes, tags='no')
        self.y = self.canvas.create_line(0, event.y, 1000, event.y, dash=dashes, tags='no')
    ...
    def onDrag(self, start, end):
        ...
        for x in self.items:
            if x not in items:
                self.canvas.itemconfig(x, fill='grey')
            else:
